...
ZSSEditor.setBaseTextColor = function(color) {
    document.getElementById('zss_field_content').style.color  = color;
}
...
ZSSEditor.init = function() {
	...
	document.addEventListener("selectionchange", function(e) {
      ...
      ZSSEditor.callback("callback-multilineSelectedtrue");
}else{

      ZSSEditor.callback("callback-multilineSelectedfalse");
}
        if(multitextSelected()){
      ZSSEditor.callback("callback-multitextSelectedtrue");
       }
       else{
       ZSSEditor.callback("callback-multitextSelectedfalse");
       }
    }, false);
    //粘贴处理。
...
ZSSEditor.insertPaste = function(plainText, currentStyle){
    plainText = plainText.replaceAll("<br/>","\n").trim();
    if(currentStyle == "paragraph"){
        //如果当前是正文段落的话，
        if(plainText.indexOf("\n") != -1){
            //对文章内容进行特殊分组处理，将文章按换行切成多个段落
            var textArray = plainText.split("\n");
            plainText = "";
            for(var i = textArray.length-1;i >= 0 ; i--){
                plainText = "<p>" + textArray[i] + "</p>" + plainText;
            }
        }
    }
    //当内容为空时，有的设备复制会出现特殊字符，这里做特殊处理
    if(ZSSEditor.getField("zss_field_content").isEmpty()){
        ZSSEditor.getField("zss_field_content").setHTML(plainText);
        //解决粘贴完之后，光标在最前，删除触发换行的bug
...
//wangxiang
ZSSEditor.setscrollTop = function(index) {
    var msg = index;
    $(document.body).scrollTop(msg);
};
ZSSEditor.getscrollTop = function() {
   var msg = $(document.body).scrollTop();
   this.log(msg);
   //$(document.body).scrollTop(msg);
   this.callback("callback-input", msg);
};
...
ZSSEditor.selectionChangedCallback = function () {
    ...
    var joinedArguments = ZSSEditor.getJoinedFocusedFieldIdAndCaretArguments();
        if(ZSSEditor.isImageSpan()){
        joinedArguments = joinedArguments + defaultCallbackSeparator + "isImageSpan=true";
        ZSSEditor.log("wangxiang",joinedArguments);
    }
...
/**
 * 当输入内容时，判断是否是imageSpan区域，如果是，则进行字数校验
 */
ZSSEditor.ImageSpanInputCheck = function(){
    if (!ZSSEditor.isImageSpan()) {
        return;
    }

    var parentNode = window.getSelection().getRangeAt(0).commonAncestorContainer.parentElement;
    //确认是imageSpan区域，进行字数校验
    if (parentNode.innerText.gblen() > 80) {
        //文字超出限制
        parentNode.style.color="red"
        //进行提示
        if(!weico.imageRecommend){
            weico.imageRecommend = true;
            ZSSEditor.callback("callback-image-oversize");
        }
    }else{
...
/**
 * 判断当前是否是imageSpan区域
 * @return Boolean [true 是imageSpan区域]
 */
ZSSEditor.isImageSpan = function(){
    //首先得到光标所在区域
    var selection = window.getSelection();
    ...
    if (parentNode.nodeName.toUpperCase() == "INPUT" && parentNode.className == "picinfo") {
        return true;
    }
    if(parentNode.nodeName.toUpperCase() == "P" && parentNode.firstChild && parentNode.firstChild.nodeName.toUpperCase() == "IMG" && parentNode.firstChild.className == "myinsert"){
        return true;
    }
...
ZSSField.prototype.handleInputEvent = function(e) {
    ...
    //
    this.emptyFieldIfNoContents();
    //在输入事件中，判断是否是图片span输入区域的文字变化。
    // ZSSEditor.ImageSpanInputCheck();
 ...
 * 确认删除选中的图片
 */
ZSSEditor.confirmImageDelete = function(){
    var imageNode = ZSSEditor.currentEditingImage;
    if(typeof(imageNode) == "undefined"){
        //没有选中的图片
        return;
    }

    var aParentNode = imageNode.parentNode;
    if (aParentNode.nodeName == "P" && aParentNode.hasAttribute("img-box")) {//判断是否是img-box的段落
        //是，进行统一移除
        aParentNode.parentNode.removeChild(aParentNode);
        return;
    }
    //段落结构已经不明确，仅删除必要的元素-img、span
    if (!imageNode.hasAttribute("data-wpid")) {
        //无法确定span，仅删除img标签
        imageNode.parentNode.removeChild(imageNode);
        return;
    }
    var imageWpid = imageNode.getAttribute("data-wpid");
    var imageSpan = document.getElementById(imageWpid);
    //删除img标签
    imageNode.parentNode.removeChild(imageNode);
    //删除span标签
    if (typeof(imageSpan) != "undefined") {
...
/**
 * 图片点击事件，点击之后选中图片，并回调到android端询问是否删除图片
 * @param  {[dom]} imageNode [当前选中的图片node]
 */
ZSSField.prototype.sendImageTappedCallback = function( imageNode ) {
    ZSSEditor.currentEditingImage = imageNode;
    //通知android端进行dialog提示
    ZSSEditor.callback('callback-image-delete');
}
...
ZSSField.prototype.getHTMLForCallback = function() {
    var allInputs = document.querySelectorAll(".picinfo");
    for (var i = allInputs.length-1; i >= 0 ; i --){
        var aInput = allInputs[i];
        //ZSSEditor.log("getAllInput " + i + " - " + aInput.value + " - " + aInput.id)
         $(aInput).attr("value",aInput.value);
         $('img[data-wpid="' + aInput.id+'"]').attr("alt",aInput.value);
    }
...
$(function(){
	//点正文滚到顶部(有内容不动，无内容则滚动到顶部)
	$("#zss_field_content").click(function(){
        var a = document.getElementById("zss_field_content").innerHTML;
        var length = a.length;
        if(length > 0){
        //if(a.endWith("<br></br>")){
                //ZSSEditor.log("nihao ture");
        //}else{
            //ZSSEditor.log("nihao false");
            //document.getElementById("zss_field_content").innerHTML= a+'<br></br>';
            //ZSSEditor.log("nihao"+document.getElementById("zss_field_content").innerHTML);
        //}
...
$(function(){
	//点title滚到顶部(有内容不动，无内容则滚动到顶部)
	$("#zss_field_title").click(function(){
        var a = document.getElementById("zss_field_title").innerHTML;
        var length = a.length;
        if(length > 0){

        }else{
        var div = document.getElementById("zss_field_title");
        var top1 = div.offsetTop;
         $('html,body').animate({scrollTop:top1}, 400);
        }
...
//选择完封面以后标题滚到最上面
ZSSEditor.coverfinishscroll = function() {
            var div = document.getElementById("zss_field_title");
            var top1 = div.offsetTop;
             $('html,body').animate({scrollTop:top1}, 400);
};
...
$(function(){
	//点用户删除事件
	$("#delete_writer").click(function(){
        $("#delete_writer").hide();
        var oStrong=document.getElementById("add_writer");
        oStrong.innerHTML='<span style="color:#939393;">添加作者</span>';
        ZSSEditor.callback("callback-delete—writer");
...
$(function(){
	...
	$("#add_writer").click(function(){
            ZSSEditor.callback("callback-add—writer");
	});
});
ZSSEditor.insertWriter = function(text) {
        $("#delete_writer").show();
        var oStrong=document.getElementById("add_writer");
        oStrong.innerHTML='<span style="color:#507daf;">'+"作者: "+text+'</span>';
};

ZSSEditor.showWriter = function() {
        $("#zss_field_writer").show();
};
ZSSEditor.hideWriter = function() {
        $("#zss_field_writer").hide();
};
...
ZSSEditor.insertLocalImage = function(imageNodeIdentifier, localImageUrl) {
    var space = '&nbsp';
    var image = '<p img-box="img-box"><img class="myinsert" data-wpid="' + imageNodeIdentifier + '" src="' + localImageUrl + '" alt="" />';
    var picinfo = '<input id="' + imageNodeIdentifier + '" class="picinfo" placeholder="图片备注" maxlength="40"></input></p>';
    var html = image + picinfo ;
...
ZSSEditor.insertHr = function() {
    ...
    var html = '<hr></hr>';  
        ...
        if(length > 0){
            ...
            this.insertHTML(space + html + space);
...
$(document).on('input propertychange paste', ".picinfo", function() {
    $('#'+$(this).attr("data-id")).attr('alt', $(this).text());
});
function multilineSelected() {
    //选区内容正文化
    var selectedText = window.getSelection().toString();
    if (selectedText.length>0) {
        if (selectedText.trim().indexOf("\n") > -1)
            return true;
    }
    return false;
};
function multitextSelected() {
    //选区内容正文化
    var selectedText = window.getSelection().toString();
    if (selectedText.length>0) {
            return true;
...
/**
 * 在添加h1 h2 Blockquotes之后，对当前选中区域所在顶级标签其内的内容进行去标签处理。
 * 在这里，顶级标签指的是(h1,h2,h3,h4,h5,p,Blockquote)
 */
ZSSEditor.removeInnerTag = function() {
    
    var _anchorNode = window.getSelection().anchorNode;
    if (null == _anchorNode) {
        //没有光标或选中
        return;
    }
    var currentNode = _anchorNode.parentElement;
    ZSSEditor.log("ZSSEditor.removeInnerTag" + currentNode.nodeName);
    var topLevels = ["P","H1","H2","H3","H4","H5","BLOCKQUOTE"];
    while (currentNode) {
        if (currentNode.nodeName.toUpperCase() == "DIV") {//异常，没有处理到任何顶级标签
            return;
        }

        //找到顶级的标签，(h1,h2,h3,h4,h5,p,Blockquote)
        if (topLevels.indexOf(currentNode.nodeName.toUpperCase()) != -1) {
            currentNode.innerHTML = currentNode.innerText
            break;
        }
        currentNode = currentNode.parentElement;
    }
};
ZSSEditor.clearRange = function(){
    window.getSelection().removeAllRanges();
};
ZSSEditor.hasHr = function() {
            obj = document.getElementById("pay_hr");
if (!obj){
        ZSSEditor.callback("callback-enter-del");
}
};

ZSSEditor.insertPayHr = function() {
    $("#pay_hr").remove();
    var space = '<p>&#x200b;</p>';
    var html = '<hr class="myhr" id="pay_hr"></hr>';
    //var html = '<img class="myinsert" id="pay_hr" src="compose_split_line.png"></img><br></br>';
            var a = document.getElementById("zss_field_content").innerHTML;
        var length = a.length;
        if(length > 0){
            //this.insertHTML(html);
            //document.getElementById("zss_field_content").innerHTML(a+html);
            $("#zss_field_content").html(a+html+space);

        }else{
            //this.insertHTML(space + html );
            //document.getElementById("zss_field_content").innerHTML(a+space+html);
            $("#zss_field_content").html(space+html+space);

        }

    this.sendEnabledStyles();
};
ZSSEditor.removeHr = function() {
    $("#pay_hr").remove();
};
